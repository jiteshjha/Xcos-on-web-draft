...
The Graphical User Interface(GUI) consists of Pallete Sidebar, Container, Toolbar and Bottom bar. Graphical User Interface employs mxGraph - An open source JavaScript diagramming component for realizing different connections between blocks and connections between them akin to that in Scilab. Pallete Browser(in Scilab) is represented as a sidebar with predefined blocks. Figure 2 shows a basic diagram on the web application. %Include Picture
...
\section{Data Structures (include the converter(amits parser), State Diagram, DS data types, new functions and data types created and operations as subsections) }
\blindtext
